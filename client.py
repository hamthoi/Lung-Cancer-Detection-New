...
# Data training is the process of training the model based on the dataset and then predict on new data.
    def send_update_to_server(self, weights, num_samples):
        ...
        try:
            ...
            if resp_json.get("status") == "success":
                messagebox.showinfo("Federated Update", f"Model update sent to server!\nTotal samples on server: {resp_json.get('total_samples')}")
            else:
    ...
    def test_image(self):
        def run_prediction():
            ...
            # --- Download global weights from server and set them ---
            try:
                response = requests.get("http://localhost:5000/download")
                if response.status_code == 200:
                    weights = [np.array(w) for w in response.json()["weights"]]
                    model.set_weights(weights)
                else:
                    self.root.after(0, lambda: messagebox.showwarning("Global Model", "Could not download global model from server. Using local weights."))
            except Exception as e:
                self.root.after(0, lambda: messagebox.showerror("Global Model", f"Failed to download global model:\n{e}"))
                return
