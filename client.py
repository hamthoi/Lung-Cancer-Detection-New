...
# Data training is the process of training the model based on the dataset and then predict on new data.
    ...
    def wait_for_global_weights(self, poll_interval=10, max_errors=3):
        ...
        while True:
            try:
                # Only poll the main server if not using backup
                if not getattr(self, "use_backup_server", False):
                    response = requests.get(self.get_server_url("download"))
                else:
                    response = requests.get(self.get_server_url("download", use_backup=True))
                if response.status_code == 200:
                    data = response.json()
                ...
                # Only show/poll errors if not using backup server
                if not getattr(self, "use_backup_server", False):
                    print(f"Error while polling for global weights: {e}")
                    consecutive_errors += 1
                else:
                    # If using backup server, just wait and retry silently
                    time.sleep(poll_interval)
                    continue

            if not getattr(self, "use_backup_server", False) and consecutive_errors >= max_errors:
                # Ask the user if they want to keep waiting, use backup, or use local
                choice = messagebox.askyesnocancel(
                    "Server Down",
                    "The server is down.\n\nYes: Keep waiting\nNo: Use local model\nCancel: Try backup server"
                )
                if choice is None:
                    # User chose Cancel: Try backup server
                    try:
                        response = requests.get(self.get_server_url("download", use_backup=True))
                        if response.status_code == 200:
                            data = response.json()
                            if data["weights"] is not None:
                                messagebox.showinfo("Backup Server", "Loaded weights from backup server. All further communication will use the backup server.")
                                self.use_backup_server = True  # Set flag
                                return [np.array(w) for w in data["weights"]]
                            else:
                                messagebox.showwarning("Backup Server", "Backup server has no weights available.")
                        else:
                            messagebox.showwarning("Backup Server", "Backup server responded with error.")
                    except Exception as e:
                        messagebox.showerror("Backup Server", f"Failed to connect to backup server:\n{e}")
                elif not choice:
                    # User chose No: Use local model
                    return None
                else:
    ...
    def get_server_url(self, endpoint, use_backup=None):
        # If use_backup is explicitly set, use it; otherwise, use the current flag
        if use_backup is None:
            use_backup = self.use_backup_server
        if use_backup:
